...
class Lasers:
    ...
    def AlienShoot(self, settings, screen, ship):
        self.lasers.add(AlienLaser(settings=settings, screen=screen, ship=ship))
    def update(self):
        self.lasers.update()
        for laser in self.lasers.copy():
            if laser.rect.bottom <= 0: self.lasers.remove(laser)
            #elif laser.rect.top >= 1200: self.lasers.remove(laser)
    def draw(self):
...
class AlienLaser(Sprite):
    ...
    def __init__(self, settings, screen, ship):
        ...
        self.y = float(self.rect.y)
        self.color = (255, randint(0, 50), randint(0, 50))
        self.speed_factor = settings.laser_speed_factor * .5
        #print(f"alien laser shot at {self.rect.centerx}, {self.y}")
    def update(self):
        self.y += self.speed_factor
        self.rect.y = self.y
        self.draw()
